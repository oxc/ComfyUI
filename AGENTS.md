...
- `docker-release.yml` builds that release by checking out the upstream tag and
  **overlaying the fork's `Dockerfile` + `.dockerignore`** onto it (the tags carry no
  ...
  never executed.
- On a successful build the tag is **recorded on origin** (with `GITHUB_TOKEN`, so it
...
- The ledger tag is an **annotated tag on the fork's `master` commit**, not a copy of
  upstream's tag object; the upstream commit it stands for is named in the tag message.
  Copying upstream's tag would push upstream commits origin does not have yet, and
  `GITHUB_TOKEN` can never push a commit that touches `.github/workflows` -- which
  fails for every release that is ahead of the last rebase. A tag on a commit origin
  already has pushes no objects, so it always succeeds.
