...
# For AMD GPUs
docker pull ghcr.io/comfyanonymous/comfyui:latest-rocm5.7

# For AMD GPUs with ROCm 6.0
docker pull ghcr.io/comfyanonymous/comfyui:latest-rocm6.0-nightly
# For CPU only
...
```docker build --build-arg PYTORCH_INSTALL_ARGS="--index-url https://download.pytorch.org/whl/rocm5.7" .```

```docker build --build-arg PYTORCH_INSTALL_ARGS="--pre --index-url https://download.pytorch.org/whl/nightly/rocm6.0" .```
